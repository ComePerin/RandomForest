...
float TreeNode::computeSplitImpurity(std::vector<int> &leftClassCounts, int nLeft,
     std::vector<int> &rightClassCounts, int nRight) const {
...
Split TreeNode::findBestSplit() const {
    ...
    for (int featureIndex = 0; featureIndex < dataSet_->n_features(); ++featureIndex) {
        ...
        for (size_t i = 0; i < samplesIndicesSortedByFeature.size() - 1; ++i) {
            ...
            float nextValue = dataSet_->getFeatureValue(samplesIndicesSortedByFeature[i + 1], featureIndex);

            if (currentValue >= nextValue) continue; // Skip redundant split

            // Compute impurity for the current split
            float splitImpurity = computeSplitImpurity(leftClassCounts, nLeft, rightClassCounts, nRight);
            ...
            if (splitImpurity < bestSplit.impurity) {
                ...
                bestSplit.threshold = (currentValue + nextValue) / 2.0f; // Midpoint between current and next value
...
void TreeNode::split(const TreeParameters &params) {
    ...
    left_ = std::make_unique<TreeNode>(dataSet_, std::move(leftIndices), depth_ + 1);
    right_ = std::make_unique<TreeNode>(dataSet_, std::move(rightIndices), depth_ +1);
...
bool TreeNode::isPure() const {
    return computeImpurity() == 0.0f;
}
